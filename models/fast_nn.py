...
class FactorAugmentedSparseThroughputNN(nn.Module):
    ...
    def __init__(self, p, r_bar, depth, width, dp_matrix, sparsity=None, rs_mat=None):
        '''
                Parameters
                ----------
                p : int
        ...
        self.bath_norm = nn.BatchNorm1d(p)
        ...
        relu_nn = [('linear1', nn.Linear(r_bar + sparsity, width)),
                   ('relu1', nn.ReLU())]
    ...
    def forward(self, x, is_training=False, **kwargs):
        ...
        if self.reconstruct is not None:
            # self.reconstruct(x1) is projection * rs_matrix, which is np.matmul(eigen_vectors, eigen_vectors.T)
            # x2 is the residual now
